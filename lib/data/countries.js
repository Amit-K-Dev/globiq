...
export async function getCountries() {
  return globiqDb.getCountries();
}

export async function getCountryById(id) {
  return globiqDb.getCountry(id) || null;
}
...
export async function searchCountries(query) {
  ...
  const countries = db.prepare("SELECT * FROM countries WHERE name LIKE ? OR region LIKE ? COLLATE NOCASE").all(q, q);
  ...
  const latestValues = db.prepare(`
    ...
    WHERE v.country_id IN (${placeholders}) AND v.year = 2023
...
export async function getCountriesWithMetricsForYear(year = 2023) {
...
export async function getCountriesWithLatestMetrics() {
  return getCountriesWithMetricsForYear(2023);
}
