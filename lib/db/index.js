...
export class GlobiqDB {
  ...
  getRankingsDynamic(metricId, year = null, limit = null) {
    const registry = this.db.prepare("SELECT * FROM metric_registry WHERE canonical_id = ?").get(metricId);
    if (!registry) return [];

    const sortDir = metricId.includes('rank') ? 'ASC' : 'DESC';

    const allowedTables = ['historical_values', 'institution_metrics', 'commodity_trade_production', 'entertainment_metrics'];
    if (!allowedTables.includes(registry.resolution_table)) return [];

    let rows = [];
    if (registry.entity_type === 'country' && registry.resolution_table === 'historical_values') {
      const yearClause = year ? 'AND v.year = ?' : 'AND v.year = (SELECT MAX(year) FROM historical_values WHERE metric_id = ?)';
      const params = year ? [metricId, year] : [metricId, metricId];
      
      let query = `
        SELECT c.*, v.${registry.resolution_column} as value
        FROM countries c
        JOIN historical_values v ON c.id = v.country_id
        WHERE v.metric_id = ? ${yearClause}
        ORDER BY v.${registry.resolution_column} ${sortDir}
      `;
      if (limit) {
        query += ` LIMIT ?`;
        params.push(limit);
    ...
    } else if (registry.entity_type === 'institution' && registry.resolution_table === 'institution_metrics') {
      const yearClause = year ? 'AND v.year = ?' : 'AND v.year = (SELECT MAX(year) FROM institution_metrics WHERE metric_id = ?)';
      const params = year ? [metricId, year] : [metricId, metricId];
      
      let query = `
        SELECT i.*, v.${registry.resolution_column} as value
        FROM institutions i
        JOIN institution_metrics v ON i.id = v.institution_id
        WHERE v.metric_id = ? ${yearClause}
        ORDER BY v.${registry.resolution_column} ${sortDir}
      `;
      if (limit) {
      ...
      rows = this.db.prepare(query).all(...params);
    } else if (registry.entity_type === 'entertainment' && registry.resolution_table === 'entertainment_metrics') {
      const yearClause = year ? 'AND v.period = ?' : "AND v.period = (SELECT MAX(CAST(period AS INTEGER)) FROM entertainment_metrics WHERE metric_id = ?)";
      const params = year ? [metricId, year.toString()] : [metricId, metricId];
      
      let query = `
        SELECT e.*, v.${registry.resolution_column} as value
        FROM entertainment_entities e
        JOIN entertainment_metrics v ON e.id = v.entity_id
        WHERE v.metric_id = ? ${yearClause}
        ORDER BY v.${registry.resolution_column} ${sortDir}
      `;
      if (limit) {
